...
use crate::{OciClient, OciConfig, SnapshotInfo, StorageError, VolumeInfo};
...
impl ZfsManager {
    ...
    pub async fn new(pool: &str) -> Result<Self, StorageError> {
        ...
        if !cli.dataset_exists(&base_dataset).await? {
            ...
            cli.set_property(&base_dataset, "compression", "zstd-3")
                .await?;
    ...
    pub async fn init_app_storage(&self, app_id: uuid::Uuid) -> Result<AppStorage, StorageError> {
        ...
        self.cli
            .create_dataset(&rootfs, Some(&format!("{}/rootfs", app_dataset)))
            .await?;
        self.cli
            .create_dataset(&data, Some(&format!("{}/data", app_dataset)))
            .await?;
    ...
    pub async fn prepare_rootfs(
        ...
        if !self.cli.dataset_exists(&cache_dataset).await? {
            ...
            self.cli
                .set_property(&cache_dataset, "compression", "zstd-3")
                .await?;
        }
        ...
        if self.cli.dataset_exists(&image_dataset).await? {
            ...
            self.pull_image_to_dataset(image_ref, &image_dataset)
                .await?;
        }
        ...
        self.cli
            .set_property(&app_rootfs, "readonly", "off")
            .await?;
    ...
    pub async fn create_volume(
        ...
        self.cli
            .set_property(&full_name, "quota", &format!("{}G", size_gb))
            .await?;
        self.cli
            .set_property(&full_name, "reservation", &format!("{}G", size_gb / 10))
            .await?; // 10% reserved
    ...
    async fn pull_image_to_dataset(
        ...
        let oci_config = OciConfig::new(self.parse_registry(image_ref));
        ...
        oci_client
            .pull_image(&reference, target_dataset, mountpoint)
            .await?;
    ...
    fn parse_registry(&self, image_ref: &str) -> String {
        ...
        if image_ref.contains('/') {
            ...
            if host_port.contains(':')
                || host_port.contains("docker.io")
                || host_port.contains("ghcr.io")
            {
