...
|---|---|---|---|---|
| A | `shellwego-registry` compiles | **FAILED** — 3 errors on older toolchain (2026-03-13 check): unresolved `bytes` import (`E0432`), borrow error in `gc()` (`E0502`), type annotation needed (`E0282`). Later build (2026-04-05) shows 0 errors but 2 warnings. | `crates/shellwego-registry/src/cache.rs` (lines 27, 318, 485-526), `Cargo.toml` | **CRITICAL** (on older toolchain) |
| B | Registry Cache / Distribution labeled "(Dragonfly)" in architecture diagram (readme line 238) | **No Dragonfly integration**. Zero Rust files reference "dragonfly". No P2P protocol, no peer discovery, no piece distribution, no scheduler. Nodes pull images independently via HTTP from upstream registries. | All crates — no `dragonfly` keyword in any `.rs` file | **CRITICAL** |
| C | Edge distribution for 100+ nodes | **No registry mirroring**. `ImagePuller` hardcodes a single upstream registry URL per image reference. No mirror list, no fallback, no geo-aware routing. If upstream is unreachable, pull fails. | `crates/shellwego-registry/src/pull.rs` (lines 384-436 `fetch_manifest`) | **HIGH** |
| D | Efficient disk usage for image cache | **Basic GC only** — `LayerCache::gc()` removes entire images by LRU on `last_accessed`. No content-addressable deduplication of shared layers. No per-layer ref-counting. Deleting one image can leave orphaned layers if another image references the same layer digest. No scheduled/background GC. | `crates/shellwego-registry/src/cache.rs` (lines 472-530) | **HIGH** |
| E | Clean architecture, no duplication | **Two duplicate OCI clients**: `ImagePuller` in `shellwego-registry/src/pull.rs` (714 lines) and `OciClient` in `shellwego-storage/src/oci.rs` (387 lines). Both implement registry auth, manifest fetching, blob download, and reference parsing independently. Different error types (`RegistryError` vs `OciError`). Different caching strategies. | `crates/shellwego-registry/src/pull.rs`, `crates/shellwego-storage/src/oci.rs` | **MEDIUM** |
| F | Pull progress reporting | `PullProgress` trait exists in `pull.rs` but is never used by the control-plane deploy handler or the agent reconciler. No streaming progress to CLI or dashboard. | `crates/shellwego-registry/src/pull.rs` (lines 69-88) | **LOW** |
