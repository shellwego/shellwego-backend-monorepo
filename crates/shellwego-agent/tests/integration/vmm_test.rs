...
async fn test_drive_config_validation() {
    let root_drive = DriveConfig::rootfs(PathBuf::from("/var/lib/shellwego/rootfs/base.ext4"));

    assert_eq!(root_drive.drive_id, "rootfs");
    assert!(root_drive.is_root_device);
    assert!(root_drive.is_read_only);

    let data_drive = DriveConfig::data(
        "vol-data",
        PathBuf::from("/dev/zvol/shellwego/volumes/data"),
        false,
    );
